...
void SceneNode::RemoveChild(SceneNode* Child)
{
	...
	{
		if (m_childrenNodes[i] == Child)
		{
			SceneNode* temp = m_childrenNodes[i];
			m_childrenNodes[i] = m_childrenNodes[m_childrenNodes.size() -1];
			m_childrenNodes[m_childrenNodes.size() - 1] = temp;
			m_childrenNodes.pop_back();
			return;
		}
	}
}
